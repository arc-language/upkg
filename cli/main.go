...
__upkg_prompt_command() {
    ...
    # Only prepend if not already there (idempotency check)
    if [[ -n "$env_prefix" ]] && [[ "$PS1" != "$env_prefix"* ]]; then
        PS1="$env_prefix$PS1"
    fi
}

# Add to PROMPT_COMMAND (Append to run last, so we override others)
if [[ -z "$PROMPT_COMMAND" ]]; then
    PROMPT_COMMAND="__upkg_prompt_command"
else
    # Check if we are already in PROMPT_COMMAND to avoid duplication on source
    if [[ "$PROMPT_COMMAND" != *"__upkg_prompt_command"* ]]; then
        PROMPT_COMMAND="${PROMPT_COMMAND%;}; __upkg_prompt_command"
    fi
fi
`)
}
